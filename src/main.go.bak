...
// TODO:: Vectorize operation to make it faster
...
func (p *Portfolio) GetBacktestingData(startTime time.Time, endTime time.Time, riskFreeRates map[int64]float64) {
	// Create the dailyAvg map for metric calculations.
...
func getTickersWithSufficientData(startTime time.Time, endTime time.Time) []string {
	...
	log.Printf("Ticker querying time: %s\n", time.Since(queryingTicker))
	return tickers
}
